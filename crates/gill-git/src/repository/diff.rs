...
pub fn diff(repository: &Repository, branch: &str, other: &str) -> anyhow::Result<String> {
            ...
            match changes.event {
                ...
                Event::Modification {
                    ...
                    | (EntryMode::BlobExecutable, EntryMode::BlobExecutable) => {
                        ...
                        let input = InternedInput::new(previous_content.as_str(), content.as_str());
...
mod test {
    ...
    fn test() -> Result<()> {
        ...
        let diffs = diff(&repo, "main", "testdiff").unwrap();
        println!("{diffs}");
        Ok(())
    }
}
